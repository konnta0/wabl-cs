...
public sealed class Cert : ConsoleAppBase
{
    ...
    public async Task AddDocker()
    {
        ...
        target.Add("cp", DependsOn("create-directory"), static () => RunAsync("sudo", $"cp -f {CertificatePath} /etc/docker/certs.d/{Domain}/"));
        target.Add("echo", DependsOn("cp"), static () => RunAsync("echo", "please docker restart!!!!"));
        await target.RunWithoutExitingAsync(new[] { "echo" });
    }
    ...
    {
        var target = new Targets();
        target.Add("sudo", static () => RunAsync("sudo", $"kubectl get secrets {SecretName} -n {SecretNamespace} -o jsonpath='{{.data.tls\\.crt}}' | base64 -D > {CertificateName})"));
        await target.RunWithoutExitingAsync(new[] { "sudo" });
    }
    ...
    {
        var target = new Targets();
        target.Add("sudo", static () => RunAsync("sudo", $"security add-trusted-cert -d -r trustAsRoot -p ssl -k /Library/Keychains/System.keychain {CertificateName}"));
        await target.RunWithoutExitingAsync(new[] { "sudo" });
    }
}